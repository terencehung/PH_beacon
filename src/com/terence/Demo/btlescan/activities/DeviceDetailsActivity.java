...
import com.terence.Demo.btlescan.util.TimeFormatter;
...
//    public static String toUtf8(byte str) throws UnsupportedEncodingException {
//        return new String(str.getBytes("UTF-8"),"UTF-8");
//    }
    
    public String StrtoUni(String str) throws UnsupportedEncodingException
    {
    String uni = "" ;
        ...
        for ( int i = 0 ; i < str.length() ; i++)
        {
            ...
            String oct = Integer.toOctalString(chr) ;
            uni += "\\u"+oct ;
        }
    String hh =toUtf8(uni);
    return hh;
    }
    private void appendAdRecordView(final MergeAdapter adapter, final String title, final AdRecord record) throws UnsupportedEncodingException {
        ...
        Log.d("BT","BT adapter ,【真實資料String】record: "+AdRecordUtils.getRecordDataAsString(record));
        Log.d("BT","BT adapter ,【真實資料S】record: "+AdRecordUtils.getServiceDataUuid(record));
        Log.d("BT","BT adapter ,【真實資料array】record: "+record.getData());
    ...
    private void pupulateDetails(final BluetoothLeDevice device) throws UnsupportedEncodingException {
...
Log.d("BT","BT adapter record.getType(): "+record.getType());
Log.d("BT","BT adapter record.getHumanReadableType(): "+record.getHumanReadableType());
Log.d("BT","BT adapter record: "+record.getData());
                    appendAdRecordView(
                            adapter,
